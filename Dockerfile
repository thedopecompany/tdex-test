...
RUN apt-get update && apt-get install -y --fix-missing wget python3 curl --no-install-recommends && \
...
# Create Python script to serve status page with logs
...
sleep 5\n\
\n\
# Start Python status server on port 3000\n\
echo "Starting web server..."\n\
cd /app && python3 status_server.py\n' > /app/start.sh
